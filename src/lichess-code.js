...
function createButton(onClick) {
  ...
  button.classList.add('centichess-button');
  ...
  text.textContent = 'Analyze in Centichess';
...
function openCentichessWithPGN(pgn) {
  window.open('https://centichess.org/?pgn=' + encodeURIComponent(pgn));
}

function insertButtonBeforeCeval(onClick) {
  const cevalEl = document.querySelector('#main-wrap > main > div.analyse__tools > div.ceval');
  if (!cevalEl || cevalEl.previousSibling?.classList?.contains('centichess-button')) return;
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            else openCentichessWithPGN(text);
          } catch (err) {
...
function handleThirdPageType() {
  insertButtonBeforeCeval(() => {
    ...
    if (pgnEl) {
      openCentichessWithPGN(pgnEl.textContent.trim());
    } else {
...
function detectPageTypeAndInsert() {
  if (document.querySelector('.centichess-button') || creatingButton) return;
  creatingButton = true;
